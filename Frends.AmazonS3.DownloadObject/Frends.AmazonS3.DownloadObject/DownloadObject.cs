...
public class AmazonS3
{
    ...
    /// <returns>Object { bool Success, List Objects { string ObjectName, string FullPath, string Overwritten, bool SourceDeleted, string Info }, Error Error { string Message, dynamic AdditionalInfo } }</returns>
    public static async Task<Result> DownloadObject([PropertyTab] Input input, [PropertyTab] Connection connection, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
    ...
    private static async Task<SingleResultObject> WriteToFile(GetObjectResponse getObjectResponse, Stream preSignedStream, AmazonS3Client amazonS3Client, S3Object fileObject, Options options, string fullPath, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                switch (options.ActionOnExistingFile)
                {
    ...
    private static bool FileLocked(int fileLockedRetries, string fullPath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            throw new Exception($"FileLocked error: {fullPath} was locked. Max Options.FileLockedRetries = {fileLockedRetries} exceeded.");
        }
        catch (Exception ex)
        {
