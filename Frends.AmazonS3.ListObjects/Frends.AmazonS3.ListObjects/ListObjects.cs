...
namespace Frends.AmazonS3.ListObjects
{
    ...
    public class AmazonS3
    {
        ...
        /// </summary>
        /// <param name="input">Input connection parameters</param>
        ...
        public static async Task<Result> ListObjects([PropertyTab] Input input, [PropertyTab] Options options, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(input.AwsSecretAccessKey) || string.IsNullOrWhiteSpace(input.AwsAccessKeyId))
                throw new Exception("AWS credentials missing.");

            var region = RegionSelection(input.Region);
            var client = new AmazonS3Client(input.AwsAccessKeyId, input.AwsSecretAccessKey, region);
