...
/// <summary>
/// Amazon S3 operations for bucket management.
/// </summary>
public class AmazonS3
{
    /// <summary>
    /// Deletes an AWS S3 bucket. The bucket must be empty before it can be deleted.
    /// If the bucket does not exist, the operation is considered successful.
    ...
    /// <param name="connection">Connection parameters for AWS authentication and region.</param>
    /// <param name="options">Options for configuring error handling behavior.</param>
    /// <param name="cancellationToken">Cancellation token generated by Frends to stop this task.</param>
    /// <returns>A Result object indicating success or failure of the bucket deletion operation.</returns>
    /// <exception cref="AmazonS3Exception">Thrown when AWS S3 service returns an error and ThrowErrorOnFailure is true.</exception>
    /// <exception cref="InvalidOperationException">Thrown when the bucket does not exist and ThrowErrorOnFailure is true.</exception>
    /// <exception cref="Exception">Thrown when an unexpected error occurs and ThrowErrorOnFailure is true.</exception>
    public static async Task<Result> DeleteBucket([PropertyTab] Input input, [PropertyTab] Connection connection, [PropertyTab] Options options, CancellationToken cancellationToken)
    {
        ...
        catch (AmazonS3Exception e)
        {
            ...
            return new Result(false, new Error(e.Message, e));
        }
        catch (Exception e)
        {
            if (options.ThrowErrorOnFailure)
